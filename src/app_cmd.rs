use crate::cuda::{NppStreamContext, cudaFree, cudaMalloc, cudaMemset2D};
...
impl GluResourceCtrlSync {
    pub fn new(
        ...
        let (cmd_sndr, cmd_rcvr) = crossbeam::channel::bounded(1);
        // let (state_sndr, state_rcvr) = crossbeam::channel::bounded(10);
        ...
        std::thread::spawn(move || {
            'outer: loop {
                log::debug!("[App] looping receiver");
                let timeout = Duration::from_secs(20);
                let mut datas = HashMap::new();
                let mut test_idx = None;
                let mut local_state = GluResourceState::Pause;
                let Some(state) = state1.upgrade() else {
                    break;
                };
                let Ok(mut state_lock) = state.write() else {
                    break;
                };
                log::debug!("[App] try to peek command {:?}", *state_lock);
                if *state_lock == GluResourceState::Pause {
                    match cmd_rcvr.try_recv() {
                        Ok(cmd) => {
                            match cmd {
                                GluResourceCommand::Start => *state_lock = GluResourceState::Start,
                                GluResourceCommand::Pause => *state_lock = GluResourceState::Pause,
                                GluResourceCommand::Stop => *state_lock = GluResourceState::Stop,
                                _ => {}
                            }
                            // let _ = state_sndr.send(cmd);
                        }
                        _ => {}
                    }
                    drop(state_lock);
                ...
                log::debug!("[App] receiving state {:?}", *state_lock);
    ...
    pub fn recv(&mut self) -> Result<HashMap<usize, CudaFrame>> {
        // match self.state_rcvr.try_recv() {
        //     Ok(GluResourceCommand::Start) => {
        //         self.pause_time.take();
        //         self.start_time.replace(Instant::now());
        //         self.previous_idx.take();
        //     }
        //     Ok(GluResourceCommand::Pause) => {
